...
이 글은 일종의 '지도' 역할로서 활용 되도록 작성했습니다. Next.js의 새로운 개념을 설명하기보다는, 여러분이 이미 알고 있는 개념들이 실제로 어떤 코드로 구현되어 있는지 찾아볼 수 있도록 안내하는 목적입니다. 궁금한 부분이 있다면 이 글을 출발점으로 삼아 직접 코드를 파헤쳐보시면 좋겠습니다.

1부에서는 리액트 18이 해결하고자 했던 문제들과 서버 컴포넌트의 특징, 그리고 React-Server-Component-Demo의 실제 구현을 다뤘습니다. 이번 2부에서는 1부의 내용을 바탕으로 Next.js가 어떻게 HTML을 생성하는지, 그리고 클라이언트에서는 어떻게 Hydration을 수행하는지 내부 코드를 보면서 이해하는 시간을 갖도록 하겠습니다.
...
서버 컴포넌트를 렌더하면 UI를 그리는데 필요한 데이터가 담긴 JS 객체를 반환합니다. 리액트 18에서는 이를 UI 데이터라 부르고, Next.js는 RSC Payload라 부릅니다. 올바른 용어는 아니지만 흔히들 Virtual Dom이라 부르기도 합니다.

1부 React-Server-Component-Demo에서 서버 컴포넌트의 결과로 생성된 UI 데이터가 서버에서 바이너리 형식으로 브라우저에 전송된다는 것을 봤습니다. 그리고 브라우저는 이 바이너리 데이터를 해석해서 자바스크립트 객체로 만든 다음, 리액트 Render 과정에서 사용합니다.
...
1. 개발자가 `npm run dev`을 실행하면 npm은 `NextRootCommand`를 실행합니다. NextRootCommand 함수는 `package/next/src/bin/next.ts`에 위치합니다. 91번째 줄 `NextRootCommand`의 인스턴스를 실행합니다. 

    153번째 줄에서 dev 실행에 대한 로직이 시작하고 204번째 줄에서 next-dev.js 모듈을 불러와서 nextDev 함수를 실행합니다.
...
2. next-dev.js 모듈 `nextDev`는 283번째 줄에서 `fork`를 활용해 새로운 Node.js 프로세스를 만듭니다. 그리고 이 프로세스는 `start-server` 코드를 실행해 Next.js 서버를 구동합니다.
...
3. `start-server` 모듈을 불러오는 즉시, 서버를 실행하는 코드가 시작됩니다. 이 로직은 427번째 줄에서부터 시작합니다. 내부의 여러 함수를 거쳐 182번째 줄에서 서버 인스턴스를 생성합니다. 그리고 395번째 줄에서 `server.listen(port, hostname)`으로 서버를 실행시킵니다.
    ```jsx
...
# 홈페이지 접속
사용자가 홈페이지(`/`)에 접속했을 때 Next.js 서버는 어떠한 과정을 거쳐 HTML과 RSC Payload를 생성하고 브라우저에게 데이터를 제공하는지 단계별로 살펴보겠습니다.
...
9. 283번째 줄 `react.hydrateRoot`은 브라우저 Dom 노드와 리액트 노드를 인자로 받습니다. 브라우저 노드에 해당하는 인자인 appElement는 33번째 줄에서 Document로 선언하였으며, reactEl은 리액트 컴포넌트와 RSC Payload가 결합되어 사용되고 있습니다.
